...
import {
  Plus, Edit2, Trash2, CheckCircle, XCircle, Save, X, Clock, Loader2,
  Scissors, Heart, Stethoscope, PawPrint, Sparkles, Star,
  Droplets, Wind, Shield, Smile, Leaf, Zap,
} from "lucide-react";
import { useApp } from "@/app/context/AppContext";
import { useAuth } from "@/app/context/AuthContext";
import { criarServico, atualizarServico, removerServico } from "@/services/api";
...
export const ICON_OPTIONS = [
  ...
  { id: "pawprint",    Icon: PawPrint,    label: "Pet" },
  { id: "sparkles",    Icon: Sparkles,    label: "Brilho" },
  { id: "star",        Icon: Star,        label: "Premium" },
  { id: "droplets",    Icon: Droplets,    label: "Banho" },
  { id: "wind",        Icon: Wind,        label: "Secagem" },
  { id: "shield",      Icon: Shield,      label: "Proteção" },
  { id: "smile",       Icon: Smile,       label: "Bem-estar" },
  { id: "leaf",        Icon: Leaf,        label: "Natural" },
  { id: "zap",         Icon: Zap,         label: "Expresso" },
] as const;
...
type ServiceForm = {
  ...
  price: number; active: boolean; icon: string;
};

const defaultForm: ServiceForm = {
  name: "", description: "", duration: 30, price: 0, active: true, icon: "scissors",
};
...
export function AdminServices() {
  ...
  const startEdit = (service: Service) => {
    ...
    setForm({
      ...
      active: service.active, icon: service.icon || "scissors",
    });
  ...
  const saveNew = async () => {
    ...
    try {
      await criarServico(
        { nome: form.name, descricao: form.description, preco: form.price, duracao_minutos: form.duration, icon: form.icon },
        adminToken
      );
  ...
  const saveEdit = async () => {
    ...
    try {
      await atualizarServico(
        editingId,
        { nome: form.name, descricao: form.description, preco: form.price, duracao_minutos: form.duration, ativo: form.active, icon: form.icon },
        adminToken
      );
      await refetchServices(); setEditingId(null);
    } catch {} finally { setSaving(false); }
  };

  const toggleActive = async (service: Service) => {
    if (!adminToken) return;
    try { await atualizarServico(service.id, { ativo: !service.active }, adminToken); await refetchServices(); } catch {}
  };

  const handleDelete = async (id: string) => {
    if (!adminToken) return;
    setDeletingId(id);
    try { await removerServico(id, adminToken); await refetchServices(); }
    catch {} finally { setDeletingId(null); setDeleteConfirmId(null); }
  };
    ...
    <div className="space-y-5">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6 border-2 border-teal-300">
          ...
          <div className="flex gap-3 mt-4">
            <button onClick={saveNew} disabled={!form.name.trim() || saving}
              className="flex items-center gap-2 bg-teal-600 text-white px-4 py-2 rounded-xl text-sm hover:bg-teal-700 disabled:opacity-50 transition-colors font-semibold">
              {saving ? <Loader2 size={14} className="animate-spin" /> : <Save size={15} />} Salvar
            </button>
            <button onClick={() => setShowAddForm(false)} className="px-4 py-2 rounded-xl text-sm border border-gray-200 text-gray-600 hover:bg-gray-50">Cancelar</button>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {services.map((service) => {
          ...
          return (
            <div key={service.id} className={`bg-white rounded-2xl shadow-sm overflow-hidden ${!service.active ? "opacity-60" : ""}`}>
              {isEditing ? (
                <div className="p-5 border-2 border-teal-400 rounded-2xl">
                  ...
                  <div className="flex gap-2 mt-4">
                    <button onClick={saveEdit} disabled={saving}
                      ...
                      {saving ? <Loader2 size={12} className="animate-spin" /> : <Save size={13} />} Salvar
                    </button>
                    <button onClick={() => setEditingId(null)} className="px-3 py-2 rounded-xl text-xs border border-gray-200 text-gray-600 hover:bg-gray-50">Cancelar</button>
                  </div>
                </div>
              ) : (
                <div className="p-5">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-4 mt-2">
                        ...
                        <span className="text-xs text-teal-700 font-semibold">R$ {service.price.toFixed(2).replace(".", ",")}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-50">
                    <button onClick={() => toggleActive(service)}
                      className={`flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-lg border transition-all font-medium ${service.active ? "border-gray-200 text-gray-500 hover:bg-gray-50" : "border-teal-200 text-teal-600 bg-teal-50 hover:bg-teal-100"}`}>
                      {service.active ? <XCircle size={12} /> : <CheckCircle size={12} />}
                      {service.active ? "Desativar" : "Ativar"}
                    </button>
                    <div className="flex items-center gap-2">
                      <button onClick={() => startEdit(service)} className="p-1.5 text-gray-400 hover:text-teal-600 hover:bg-teal-50 rounded-lg transition-all" title="Editar">
                        <Edit2 size={14} />
                      </button>
                      {deleteConfirmId === service.id ? (
                        <div className="flex gap-1">
                          ...
                          <button onClick={() => setDeleteConfirmId(null)} className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-lg">Não</button>
                        </div>
                      ) : (
                        <button onClick={() => setDeleteConfirmId(service.id)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all" title="Excluir">
                          <Trash2 size={14} />
                        </button>
                      )}
                    </div>
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      <div className="sm:col-span-2">
        ...
        <input type="text" value={form.name} onChange={(e) => update("name", e.target.value)} placeholder="Ex: Banho Completo"
      ...
      <div>
        ...
        <input type="number" value={form.duration} onChange={(e) => update("duration", Number(e.target.value))} min={5} step={5}
          className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-teal-400" />
      </div>
      <div>
        <label className="block text-xs text-gray-600 mb-1 font-semibold">Preço (R$)</label>
        <input type="number" value={form.price} onChange={(e) => update("price", Number(e.target.value))} min={0} step={0.01}
          className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-teal-400" />
      </div>
      <div className="sm:col-span-2">
        <label className="block text-xs text-gray-600 mb-2 font-semibold">Ícone</label>
        <div className="grid grid-cols-6 gap-2">
          {ICON_OPTIONS.map(({ id, Icon, label }) => (
            <button key={id} type="button" onClick={() => update("icon", id)} title={label}
              className={`flex flex-col items-center gap-1 p-2 rounded-xl border-2 transition-all ${
                form.icon === id ? "border-teal-500 bg-teal-50" : "border-gray-200 hover:border-teal-300"
              }`}>
              <Icon size={18} className={form.icon === id ? "text-teal-600" : "text-gray-500"} />
      ...
      <div className="flex items-center gap-2">
        <input type="checkbox" id="active-check" checked={form.active} onChange={(e) => update("active", e.target.checked)} className="accent-teal-600 w-4 h-4" />
        <label htmlFor="active-check" className="text-sm text-gray-700 cursor-pointer">Serviço ativo (visível aos clientes)</label>
      </div>
    </div>
  );
}
